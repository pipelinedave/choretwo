...
const USER = specEmail("chore-crud");
test.describe("Chore CRUD + Undo Flow", () => {
  ...
  test.beforeEach(async ({ context, page }) => {
    ...
    await e2eLogin(page, { email: USER, name: "chore-crud" });
  });
  ...
  test("should undo chore creation via API", async ({ page, request }) => {
    ...
    const createLog = logs.find(
      (log) =>
        log.action_type === "created" && log.action_details?.name === choreName,
    );
  ...
  test("should show logs after chore actions", async ({ page, request }) => {
    ...
    const createLog = logs.find(
      (log) => log.action_details?.name === choreName,
    );
    expect(createLog).toBeTruthy();
