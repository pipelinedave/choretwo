...
test.describe('Undo marked_done Bug (Issue #42)', () => {
  ...
  test('undo marked_done should reset due_date to previous value (not next period)', async ({ page, request }) => {
    const testId = Date.now()
    const choreName = `Undo Bug Test Chore ${testId}`
    // Create chore with due_date = today
    ...
    const createResp = await request.post('/api/chores/', {
      ...
      data: { name: choreName, interval_days: 7, due_date: today, is_private: false }
    })
    const choreData = await createResp.json()
    expect(createResp.status()).toBe(200)
    const choreId = choreData.id
    
    // 2. Mark as done → this should set: done=true, last_done=today, due_date=nextWeek
    ...
    expect(afterDoneChore.due_date).toBe(nextWeek) // BUG: due_date moved forward
    ...
    const doneLog = logs.find(l => l.action_type === 'marked_done' && l.chore_id === choreId)
    expect(doneLog).toBeTruthy('marked_done log entry not found')
    ...
    // After undo: done=false, last_done=null, due_date=original (today)
    // BUG: due_date stays at nextWeek
    expect(afterUndoChore.done).toBe(false, 'After undo: chore should be marked as incomplete')
    expect(afterUndoChore.due_date).toBe(today, 'After undo: due_date should be reset to original (today), stayed at nextWeek')
    expect(afterUndoChore.last_done).toBeNull(), 'After undo: last_done should be null'
  })
})
