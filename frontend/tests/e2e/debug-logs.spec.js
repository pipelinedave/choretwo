...
const USER = specEmail("debug-logs");
test("debug: what action_types exist for done events?", async ({
  page,
  request,
}) => {
  await e2eLogin(page, { email: USER, name: "debug-logs" });
  ...
  for (const log of logs) {
    if (
      log.action_details?.chore_id ||
      log.action_type === "completed" ||
      log.action_details?.done
    ) {
      console.log(
        `DONE-RELATED: type=${log.action_type}, chore_id=${log.chore_id}, details=${JSON.stringify(log.action_details)}`,
      );
    }
  }
});
